...
def test_shell_imports_only_create_app_from_the_two_mounted_apps():
    """From dispatch.queue.app and dispatch.reports.app specifically --
    their own Flask modules -- the shell imports exactly one name:
    create_app. Everything else it uses (QueueStore, the reports read
    functions) comes from those lanes' own separate, legitimate public
    modules (store.py, snapshot.py, queries.py, readonly.py), not from
    reaching into the .app module's internals."""
    import ast
    ...
    for node in ast.walk(tree):
        if isinstance(node, ast.ImportFrom) and node.module in ("dispatch.queue.app", "dispatch.reports.app"):
